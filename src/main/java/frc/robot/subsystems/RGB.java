...
public class RGB {
    ...
    public void periodic() {
        ...
        if (ledFiler.calculate(pdh.getVoltage()) < 10
                || (robot.isDisabled() && ledFiler.calculate(pdh.getVoltage()) < 12)) {
            // low battery warning
            rgbControl.set(Constants.RGB.LED_MODE_HEARTBEAT_RED);
        } else if (robot.isDisabled()) {
            // robot disabled
            rgbControl.set(Constants.RGB.LED_MODE_OFF);
        } else if (DriverStation.getMatchTime() < 22 && DriverStation.getMatchTime() > 18) {
            // end game
